"""SDL3 autoconf checks for embedded Linux and custom configurations.

This file provides configuration checks that can be used with rules_cc_autoconf
for dynamic feature detection on embedded Linux systems or custom build environments.

Usage (for embedded Linux with dynamic detection):
    bazel build //:SDL3 --//:autoconf=True

The default build uses hardcoded defines for stability. Enable autoconf when:
- Cross-compiling for embedded systems with varying feature availability
- Building for custom Linux distributions
- Need to detect features at configure time
"""
...
# Unix-specific checks (Linux, macOS, etc.)
SDL3_CONFIG_CHECKS_UNIX = SDL3_CONFIG_CHECKS_COMMON + [
    ...
    checks.AC_CHECK_FUNC("setenv", define = "HAVE_SETENV"),
    checks.AC_CHECK_FUNC("putenv", define = "HAVE_PUTENV"),
    checks.AC_CHECK_FUNC("unsetenv", define = "HAVE_UNSETENV"),
...
SDL3_CONFIG_CHECKS_LINUX = SDL3_CONFIG_CHECKS_UNIX + [
    checks.AC_CHECK_HEADER("linux/input.h", define = "HAVE_LINUX_INPUT_H"),
    checks.AC_CHECK_FUNC("getresuid", define = "HAVE_GETRESUID"),
    checks.AC_CHECK_FUNC("getresgid", define = "HAVE_GETRESGID"),
    ...
    # Process
    checks.AC_DEFINE("SDL_PROCESS_POSIX", "1"),
]

# Linux X11 checks (optional)
...
# Linux OpenGL ES checks (optional)
...
SDL3_CONFIG_CHECKS_MACOS = SDL3_CONFIG_CHECKS_UNIX + [
    ...
    checks.AC_DEFINE("SDL_GPU_METAL", "1"),
    ...
    checks.AC_DEFINE("SDL_CAMERA_DRIVER_COREMEDIA", "1"),
...
SDL3_CONFIG_CHECKS_WINDOWS = SDL3_CONFIG_CHECKS_COMMON + [
    ...
    checks.AC_DEFINE("SDL_THREAD_GENERIC_RWLOCK_SUFFIX", "1"),
    checks.AC_DEFINE("SDL_TIMER_WINDOWS", "1"),
    checks.AC_DEFINE("SDL_TIME_WINDOWS", "1"),
    checks.AC_DEFINE("SDL_POWER_WINDOWS", "1"),
    checks.AC_DEFINE("SDL_FILESYSTEM_WINDOWS", "1"),
    checks.AC_DEFINE("SDL_FSOPS_WINDOWS", "1"),
    checks.AC_DEFINE("SDL_SENSOR_WINDOWS", "1"),
    checks.AC_DEFINE("SDL_PROCESS_WINDOWS", "1"),
    checks.AC_DEFINE("SDL_CAMERA_DRIVER_MEDIAFOUNDATION", "1"),
    checks.AC_DEFINE("SDL_CAMERA_DRIVER_DUMMY", "1"),
    checks.AC_DEFINE("SDL_GPU_D3D11", "1"),
    checks.AC_DEFINE("SDL_GPU_D3D12", "1"),
    checks.AC_DEFINE("SDL_GPU_VULKAN", "1"),
    checks.AC_DEFINE("SDL_VIDEO_VULKAN", "1"),
    checks.AC_DEFINE("SDL_VIDEO_RENDER_D3D", "1"),
    checks.AC_DEFINE("SDL_VIDEO_RENDER_D3D11", "1"),
    checks.AC_DEFINE("SDL_VIDEO_RENDER_D3D12", "1"),
    checks.AC_DEFINE("SDL_VIDEO_RENDER_OGL", "1"),
    checks.AC_DEFINE("SDL_VIDEO_RENDER_GPU", "1"),
    checks.AC_DEFINE("SDL_VIDEO_OPENGL", "1"),
